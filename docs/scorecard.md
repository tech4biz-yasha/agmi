...
- `tamper`: reported. detected on reload (verify_writes: ['memory 430d7b37bb: its TEXT or KEY no longer matches its write receipt (edited after write)']).
...
- `reorder`: reported. detected on reload (verify_writes: ['memory 86da57ceb8: its TEXT or KEY; its VALUE (`object`); WHEN the fact became true (`valid_from`), or where that time came from no longer).
- `forge`: reported. detected on reload (verify_writes: ["1 record(s) are covered by NO write receipt, so nothing here vouches for them: ['f0b0007ad1']. They were inserted out of band, or written ).
- `cross_replay`: accepted. accepted silently.
- `rollback_replay`: reported. detected on reload (verify_writes: ['memory d0049c47e2: its TEXT or KEY; its VALUE (`object`); WHEN the fact became true (`valid_from`), or where that time came from no longer).
- `metadata_tamper`: reported. detected on reload (verify_writes: ['memory c6a4a209ab: a field its receipt commits to no longer matches its write receipt (edited after write)']).

### inspeximus-rcpt+dir+home

- `tamper`: reported. detected on reload (verify_writes: ['memory 381c93713c: its TEXT or KEY no longer matches its write receipt (edited after write)']).
- `truncate`: accepted. accepted silently.
- `delete_middle`: reported. detected on reload (verify_writes: ['receipt 2: broken chain link (a prior receipt was altered/removed)']).
- `reorder`: reported. detected on reload (verify_writes: ['memory ac000fa9f9: its TEXT or KEY; its VALUE (`object`); WHEN the fact became true (`valid_from`), or where that time came from no longer).
- `forge`: reported. detected on reload (verify_writes: ["1 record(s) are covered by NO write receipt, so nothing here vouches for them: ['f0e8f4f7e6']. They were inserted out of band, or written ).
- `cross_replay`: accepted. accepted silently.
- `rollback_replay`: reported. detected on reload (verify_writes: ['memory e5d1ff40cb: its TEXT or KEY; its VALUE (`object`); WHEN the fact became true (`valid_from`), or where that time came from no longer).
- `metadata_tamper`: reported. detected on reload (verify_writes: ['memory 2cf49919c3: a field its receipt commits to no longer matches its write receipt (edited after write)']).
