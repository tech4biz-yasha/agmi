...
- `retrieval_hijack`: surfaced. attacker entry took a slot from a genuine memory (external: 4 of 5, ranks 2, out, 1, 2, 1 of 3; laundered: 4 of 5, ranks 2, out, 1, 2, 1 of 3; agent-laundered: 4 of 5, ranks 2, out, 1, 2, 1 of 3).
...
- `tamper`: reported. detected on reload (verify_writes: ['memory 7628939cb5: its TEXT or KEY no longer matches its write receipt (edited after write)']).
...
- `reorder`: reported. detected on reload (verify_writes: ['memory d49d4e54b9: its TEXT or KEY; its VALUE (`object`); WHEN the fact became true (`valid_from`), or where that time came from no longer).
- `forge`: reported. detected on reload (verify_writes: ["1 record(s) are covered by NO write receipt, so nothing here vouches for them: ['f068663386']. They were inserted out of band, or written ).

### inspeximus-rcpt+dir+home

- `tamper`: reported. detected on reload (verify_writes: ['memory bb58e2b2e3: its TEXT or KEY no longer matches its write receipt (edited after write)']).
- `truncate`: accepted. accepted silently.
- `delete_middle`: reported. detected on reload (verify_writes: ['receipt 2: broken chain link (a prior receipt was altered/removed)']).
- `reorder`: reported. detected on reload (verify_writes: ['memory 0f0b3c409f: its TEXT or KEY; its VALUE (`object`); WHEN the fact became true (`valid_from`), or where that time came from no longer).
- `forge`: reported. detected on reload (verify_writes: ["1 record(s) are covered by NO write receipt, so nothing here vouches for them: ['f0bbdba2a0']. They were inserted out of band, or written ).
